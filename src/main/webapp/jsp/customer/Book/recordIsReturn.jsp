...
<div class="formbody">
    ...
    <table class="tablelist">
        ...
        <tbody>
    <c:forEach var="i" items="${myBorrowingIsReturnRecord}">
            <tr>
                <td class="img_td">
...
<div class="page_container center">
    <c:choose>
        <c:when test="${indexPage==0} ||${indexPage<0}">
            ...
            <div class="page_btn prev_page left" ><a class="page_btn prev_page left" href="<%=basePath%>user/myBorrowingIsReturnRecord?index=0">上一页</a></div>

        </c:when>

        <c:when test="${indexPage>0}">
            <div class="page_btn prev_page left" ><a class="page_btn prev_page left" href="<%=basePath%>user/myBorrowingIsReturnRecord?index=${indexPage-1}">上一页</a></div>
    ...
    <div class="page_num_container left" id="page_num_container">
        <ul>
            <c:forEach var="i" begin="1" end="${PageCount}" step="1">
                <li><a href="<%=basePath%>user/myBorrowingIsReturnRecord?index=${i-1}">
                <li>${i}</li>
                </a></li>
            </c:forEach>
        </ul>
    ...
    <c:choose>
        <c:when test="${indexPage==PageCount-1}">
            <div class="page_btn next_page left"><a href="<%=basePath%>user/myBorrowingIsReturnRecord?index=${PageCount-1}">下一页</a></div>

        </c:when>
        <c:when test="${indexPage<PageCount-1}">
            <div class="page_btn next_page left"><a href="<%=basePath%>user/myBorrowingIsReturnRecord?index=${indexPage+1}">下一页</a></div>
