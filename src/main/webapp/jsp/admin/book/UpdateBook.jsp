        ...
        $(document).ready(function () {
            ...
            $("#submit_button").click(function () {
                ...
                if (status == true) { //如果表单验证通过
                    $.post("<%=basePath%>admin/toUpdateBook",$("#UpdateBookForm").serialize(),function (data) {
                        if(data.massage==1){
...
<div class="formbody">
    ...
    <form id="UpdateBookForm" method="post ">
        <ul class="forminfo">
            <li> <img  id="book_image" style="margin-left: 90px" border="1"  width="180px"  height="180px" src="${getBook.book_icon}">
            ...
            <li>
                ...
                <select name="book_type.id" class="dfinput" id="book_type">
                    ...
                    <c:forEach var="i" items="${getBookType}">
                        ...
                        <c:choose>
                            <c:when test="${getBook.book_type.id==i.id}">
                                <option value="${i.id}" selected="selected">${i.book_type_name}</option>
                            </c:when>
                            <c:otherwise>
           ...
           <input id="id" name="id" value="${getBook.id}" type="text"  hidden class="dfinput"/>




            <li><label>*书名</label><input id="book_name" name="book_name" type="text" value="${getBook.book_name}" class="dfinput"/><i
                    id="name_message"></i></li>
            <li><label>*作者</label><input id="book_author" name="book_author" type="text" value="${getBook.book_author}" class="dfinput"/><i
                    id="author_message"></i></li>
            <input id="book_icon" name="book_icon" type="text" class="dfinput" value="${getBook.book_icon}"  readonly="readonly" style="display:none" />

            <li><label>*书的数量</label><input id="book_number" name="book_number" value="${getBook.book_number}" type="text" class="dfinput"/><i
                    id="number_message"></i></li>
            <li><label>书本评论</label>
                <textarea id="book_description" name="book_description"  cols="" rows="" class="textinput">${getBook.book_description}</textarea><i></i>
            </li>

            <li><label>&nbsp;</label><input id="submit_button" type="button" class="btn" value="修改书籍"/></li>
        </ul>
    </form>
</div>
</body>
